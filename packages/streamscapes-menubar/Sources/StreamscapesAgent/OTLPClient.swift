...
struct OTLPClient {
    ...
    func postSpan(
        ...
        guard let url = URL(string: "\(endpoint)?source=\(source)"),
        ...
        request.setValue(kAgentVersion, forHTTPHeaderField: "X-Agent-Version")
        request.httpBody = jsonData

        do {
            let (_, response) = try await URLSession.shared.data(for: request)
            let statusCode = (response as? HTTPURLResponse)?.statusCode ?? 0
            return (200...299 ~= statusCode, statusCode)
        } catch {
            return (false, nil)
        }
    }
